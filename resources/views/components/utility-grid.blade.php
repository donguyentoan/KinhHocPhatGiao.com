...
<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
    @foreach($utilities as $utility)
        ...
        @endphp
        @if(filled($href))
            <a href="{{ $href }}" class="group bg-white/60 border border-gray-300 rounded-2xl p-5 flex flex-col items-center text-center transition-all duration-300 hover:bg-white hover:shadow-xl hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-[#8b5e34]/40">
        @else
